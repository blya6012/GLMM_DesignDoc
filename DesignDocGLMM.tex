...
\section{.C}
To transfer the \texttt{pql} package to C, the \texttt{.C} command will be used. This command takes all the arguments needed for the C function as inputs, and calls the C file. The C file is then executed, and at completeion, returns the results back to R. 

\section {Inner}
The first part to be transfered to C is the inner function. The inner function caluclates the Hessian matrix, which is then used in the outer function optimization.
\end{document}
